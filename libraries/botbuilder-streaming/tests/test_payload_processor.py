...
import aiounittest
from botbuilder.streaming import ReceiveRequest
from botbuilder.streaming.payloads import StreamManager
from botbuilder.streaming.payloads.assemblers import ReceiveRequestAssembler, PayloadStreamAssembler
from botbuilder.streaming.payloads.models import Header, RequestPayload, StreamDescription
...
class TestPayloadProcessor(aiounittest.AsyncTestCase):
    async def test_process_request(self):
        ...
        sut = ReceiveRequestAssembler(header, stream_manager, on_completed=mock_on_completed)

        # Act
        stream_id: UUID = uuid4()
        streams: List[StreamDescription] = [StreamDescription(id=str(stream_id), content_type="json", length=100)]
        payload = RequestPayload(verb="POST", path="/api/messages", streams=streams).to_json()
