...
class TeamsInfo:
    ...
    def get_team_details(turn_context: TurnContext, team_id: str = "") -> TeamDetails:
        ...
        return TeamsInfo.get_teams_connector_client(turn_context).teams.get_team_details(team_id)

    @staticmethod
    def get_team_channels(turn_context: TurnContext, team_id: str = "") -> List[ChannelInfo]:
        if not team_id:
            team_id = TeamsInfo.get_team_id(turn_context)
        ...
        return TeamsInfo.get_teams_connector_client(turn_context).teams.get_teams_channels(team_id).conversations
    ...
    async def get_team_members(turn_context: TurnContext, team_id: str = ""):
        ...
        return await TeamsInfo._get_members(TeamsInfo._get_connector_client(turn_context), turn_context.activity.conversation.id)
    ...
    async def get_members(turn_context: TurnContext) -> List[TeamsChannelAccount]:
        ...
        if not team_id:
            ...
            return await TeamsInfo._get_members(TeamsInfo._get_connector_client(turn_context), conversation_id)
    ...
    def get_teams_connector_client(turn_context: TurnContext) -> TeamsConnectorClient:
        ...
        return TeamsConnectorClient(connector_client.config.credentials, turn_context.activity.service_url)
    ...
    async def _get_members(connector_client: ConnectorClient, conversation_id: str) -> List[TeamsChannelAccount]:
        ...
        teams_members = []
        members = await connector_client.conversations.get_conversation_members(conversation_id)
