...
class TestTelemetryMiddleware(aiounittest.AsyncTestCase):
    ...
    async def test_should_send_receive(self):
        ...
        #     value (generated)
        telemetry_calls = [
            (TelemetryLoggerConstants.BOT_MSG_RECEIVE_EVENT, {"text":"foo", "fromId": "User1", "conversationName": None, "locale":None, 
                                                              "recipientId" : "bot", "recipientName": 'user', }),
            (TelemetryLoggerConstants.BOT_MSG_SEND_EVENT, {"text": "echo:foo", "replyActivityId": None, "recipientId": None,
                                                           "conversationName" : None, "locale": None, }),
            (TelemetryLoggerConstants.BOT_MSG_RECEIVE_EVENT, {'text': 'bar', 'fromId': 'User1', 'conversationName': None, 'locale': None,
                                                           'recipientId': 'bot', 'recipientName': 'user', 'fromName': 'user'}),
            (TelemetryLoggerConstants.BOT_MSG_SEND_EVENT, {'replyActivityId': None, 'recipientId': 'User1', 'conversationName': None, 
                                                           'locale': None, 'fromName': 'Bot', 'text': 'echo:bar'}),
        ]
    ...
    async def test_log_update(self):
        ...
        telemetry_call_expected = [
            (TelemetryLoggerConstants.BOT_MSG_RECEIVE_EVENT, {"text":"foo", "fromId": "User1", "conversationName": None, "locale":None, 
                                                              "recipientId" : "bot", "recipientName": 'user', }),
            (TelemetryLoggerConstants.BOT_MSG_SEND_EVENT, {'replyActivityId': '1', 'recipientId': 'User1', 'conversationName': None,
                                                           'locale': None, 'fromName': 'Bot', 'text': 'response'}),
            (TelemetryLoggerConstants.BOT_MSG_RECEIVE_EVENT, {'text': 'update', 'fromId': 'User1', 'conversationName': None, 'locale': None,
                                                              'recipientId': 'bot', 'recipientName': 'user', 'fromName': 'user'}),
            (TelemetryLoggerConstants.BOT_MSG_UPDATE_EVENT, {'recipientId': 'User1', 'conversationId': 'Convo1', 'conversationName': None,
                                                             'locale': None, 'text': 'new response'}),
        ]
    ...
    def assert_telemetry_call(self, telemetry_mock, index: int, event_name: str,\
                              props: Dict[str, str]) -> None:
        # pylint: disable=unused-variable

        if index > len(telemetry_mock.track_event.call_args_list) - 1:
            self.assertTrue(False, f'{len(telemetry_mock.track_event.call_args_list)} calls were made. You were asking for index {index}.')
        args, kwargs = telemetry_mock.track_event.call_args_list[index]
        self.assertEqual(args[0], event_name, f'Event NAME not matching.\n   Expected: {props}\n   Generated: {args[1]}')
        ...
        for key, val in props.items():
            ...
            self.assertTrue(isinstance(args[1], dict), f'ERROR: Second parm passed not a dictionary! {type(args[1])}')
            if props[key]:
                self.assertTrue(val == args[1][key], f'   ERROR: Validate failed: "{val}" expected, "{args[1][key]}" generated')
