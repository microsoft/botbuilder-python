...
class SimpleAdapter(BotAdapter):
    ...
    async def delete_activity(self, context: TurnContext, reference: ConversationReference):
        self.test_aux.assertIsNotNone(reference, "SimpleAdapter.delete_activity: missing reference")
        if self._call_on_delete is not None: self._call_on_delete(reference)
    
    async def send_activities(self, context: TurnContext, activities: List[Activity]):
        self.test_aux.assertIsNotNone(activities, "SimpleAdapter.delete_activity: missing reference")
        self.test_aux.assertTrue(len(activities) > 0, "SimpleAdapter.send_activities: empty activities array.")

        if self._call_on_send is not None: self._call_on_send(activities)
        responses = []
    ...
    async def update_activity(self, context: TurnContext, activity: Activity):
        self.test_aux.assertIsNotNone(activity, "SimpleAdapter.update_activity: missing activity")
        if self._call_on_update is not None: self._call_on_update(activity)
