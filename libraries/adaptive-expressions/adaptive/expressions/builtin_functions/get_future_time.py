...
class GetFutureTime(ExpressionEvaluator):
    def __init__(self):
        super().__init__(
            GETFUTURETIME, GetFutureTime.evaluator, ReturnType.String, GetFutureTime.validator
        )
    ...
    def evaluator(expression: object, state, options: Options):
        ...
        if error is None:
            if isinstance(args[0], numbers.Number) and isinstance(args[1], str):
                ...
                time_converter, error = FunctionUtils.date_time_converter(args[0], args[1], False)
                if error is None:
                    value = time_converter(datetime.utcnow()).strftime(time_format)
                    if len(args) != 3:
                        value = value[:-4] + "Z"
                else:
                    error = "{" + expression.to_string() + "} should contain a time interval integer" + \
                        ", a string unit of time and an optional output format string."
        return value, error

    @staticmethod
    def validator(expression: object):
        FunctionUtils.validate_order(expression, [ReturnType.String], ReturnType.Number, ReturnType.String)
