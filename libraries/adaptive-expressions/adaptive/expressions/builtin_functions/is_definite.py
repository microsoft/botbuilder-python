...
class IsDefinite(ExpressionEvaluator):
    def __init__(self):
        super().__init__(
            ISDEFINITE, IsDefinite.eval, ReturnType.String, FunctionUtils.validate_unary,
        )
    ...
    def eval(expression: object, state: MemoryInterface, options: Options):
        ...
        if error is None:
            value = parsed is not None and parsed.year is not None and parsed.month is not None and parsed.day_of_month is not None
