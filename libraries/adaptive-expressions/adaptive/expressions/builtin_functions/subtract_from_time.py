...
class SubtractFromTime(ExpressionEvaluator):
    def __init__(self):
        super().__init__(
            SUBTRACTFROMTIME, SubtractFromTime.evaluator, ReturnType.String, SubtractFromTime.validator
        )
    ...
    def evaluator(expression: object, state, options: Options):
        ...
        if error is None:
            if isinstance(args[1], numbers.Number) and isinstance(args[2], str):
                ...
                time_converter, error = FunctionUtils.date_time_converter(args[1], args[2])
                if error is None:
                    def anonymous_function(date_time: datetime):
                        return time_converter(date_time).strftime(time_format)[:-4] + "Z", None
                    value, error = FunctionUtils.normalize_to_date_time(args[0], anonymous_function)
            else:
