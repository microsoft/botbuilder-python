...
class IsDateRange(ExpressionEvaluator):
    ...
    def eval(expression: object, state: MemoryInterface, options: Options):
        ...
        if parsed is not None and error is None:
            value = (
                (parsed.year is not None and parsed.day_of_month is None)
                or (
                    parsed.year is not None
                    and parsed.month is not None
                    and parsed.day_of_month is None
                )
                or (parsed.month is not None and parsed.day_of_month is None)
                or parsed.season is not None
                or parsed.week_of_year is not None
                or parsed.week_of_month is not None
            )
