...
class SetPathToValue(ExpressionEvaluator):
    ...
    def evaluator(expression: object, state, options: Options):
        path, left, error = FunctionUtils.try_accumulate_path(expression.children[0], state, options)
        if error is not None:
            return None, error
        if left is not None:
            # the expression can't be fully merged as a path
            return None, "{" + expression.children[0].to_string() + "is not a valid path to set value."
