...
from botframework.schema.teams import models
from .operations.teams_operations import TeamsOperations
from ._teams_connector_client_configuration import TeamsConnectorClientConfiguration


class TeamsConnectorClient(SDKClient):
    """﻿﻿The Bot Connector REST API extension for Microsoft Teams allows your bot to perform extended operations
    on to Microsoft Teams channel configured in the[Bot Framework Developer Portal](https://dev.botframework.com).
    The Connector service uses industry-standard REST and JSON over HTTPS. Client libraries for this REST API are
    available. See below for a list. Authentication for both the Bot Connector and Bot State REST APIs is
    accomplished with JWT Bearer tokens, and is described in detail in the
    [Connector Authentication](https://docs.botframework.com/en-us/restapi/authentication) document.
