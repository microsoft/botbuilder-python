...
from botframework.connector import HttpClientFactory, Channels
...
class _ParameterizedBotFrameworkAuthentication(BotFrameworkAuthentication):
    def __init__(
        ...
        self._to_bot_from_channel_open_id_metadata_url = to_bot_from_channel_open_id_metadata_url
        self._to_bot_from_emulator_open_id_metadata_url = to_bot_from_emulator_open_id_metadata_url
    ...
    def create_bot_framework_client(self) -> BotFrameworkClient:
        return _BotFrameworkClientImpl(
            self._credentials_factory, self._http_client_factory, self._to_channel_from_bot_login_url, self._logger
        )
    ...
    async def _jwt_token_validation_validate_claims(self, claims: Dict[str, object]):
        if self._auth_configuration.claims_validator:
            ...
            raise PermissionError("ClaimsValidator is required for validation of Skill Host calls.")
    ...
    async def _emulator_validation_authenticate_emulator_token(
        ...
        if not version_claim or version_claim == "1.0":
            ...
            if not app_id:
                ...
                raise PermissionError("'appid' claim is required on Emulator Token version '1.0'.")
        elif version_claim == "2.0":
            app_id = identity.get_claim_value(AuthenticationConstants.AUTHORIZED_PARTY)
            if not app_id:
                raise PermissionError("'azp' claim is required on Emulator Token version '2.0'.")
        else:
    ...
    async def _government_channel_validation_authenticate_channel_token(
        ...
        await self._government_channel_validation_validate_identity(identity, service_url)
    ...
    async def _government_channel_validation_validate_identity(
        ...
        if service_url is not None:
            service_url_claim = identity.get_claim_value(AuthenticationConstants.SERVICE_URL_CLAIM)
