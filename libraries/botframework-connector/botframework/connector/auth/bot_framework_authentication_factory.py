...
from botframework.connector.auth._parameterized_bot_framework_authentication import (
    _ParameterizedBotFrameworkAuthentication
)
from botframework.connector.auth._public_cloud_bot_framework_authentication import (
...
class BotFrameworkAuthenticationFactory:
    ...
    def create(
        *,
        ...
        :param http_client_factory: The HttpClientFactory to use.
        :param logger: The Logger to use.
        :return: A new BotFrameworkAuthentication instance.
        """
        if (
            to_channel_from_bot_login_url or
            to_channel_from_bot_oauth_scope or
            to_bot_from_channel_token_issuer or
            oauth_url or
            to_bot_from_channel_open_id_metadata_url or
            to_bot_from_emulator_open_id_metadata_url or
            caller_id
        ):
            ...
            if not channel_service:
                return _PublicCloudBotFrameworkAuthentication(
                    credential_factory, auth_configuration, http_client_factory, logger
                )
