...
class EmulatorApiClient:
    @staticmethod
    async def emulate_oauth_cards(credentials: MicrosoftAppCredentials, emulator_url: str, emulate: bool) -> bool:
        token = await credentials.get_token()
        request_url = emulator_url + ('' if emulator_url[-1] == '/' else '/') + f'api/usertoken/emulateOAuthCards?emulate={ str(emulate).lower() }'
