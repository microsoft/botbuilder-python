...
class ComponentDialog(Dialog):
    ...
    def __init__(self, dialog_id: str):
        ...
        :param dialog_id: The ID to assign to the new dialog within the parent
        dialog set.
        :type dialog_id: str
        """
    ...
    async def begin_dialog(
        ...
        :param dialog_context: The :class:`DialogContext` for the current turn
        of the conversation.
    ...
    async def continue_dialog(self, dialog_context: DialogContext) -> DialogTurnResult:
        """
        Called when the dialog is continued, where it is the active dialog and
        the
        ...
        .. remarks::
            ...
            is still active after the turn has been processed by the dialog.
            The result may also contain a return value.
            ...
            and if a :class:`DialogTurnResult.result` is available, the component
            dialog uses that as it's return value.


        :param dialog_context: The parent :class:`DialogContext` for the current
    ...
    async def resume_dialog(
        ...
        turn, returning control to this dialog component.

        .. remarks::

            Containers are typically leaf nodes on the stack but the dev is free
            to push other dialogs on top of the stack which will result in the
            container receiving an unexpected call to
            :meth:`DialogContext.resume_dialog()` when the pushed on dialog ends.
            To avoid the container prematurely ending we need to implement this
            method and simply ask our inner dialog stack to re-prompt.
            ...
            :meth:`DialogContext.begin_dialog()`in the parent's context. However,
             if the :meth:`DialogContext.replace_dialog()` method is called, the
    ...
    async def on_begin_dialog(
        ...
        .. remarks::
            ...
            By default, this calls the :meth:`Dialog.begin_dialog()` method of
            the component dialog's initial dialog.

            Override this method in a derived class to implement interrupt logic.

        :param inner_dc: The inner :class:`DialogContext` for the current turn
        of conversation.
    ...
    async def on_end_dialog(  # pylint: disable=unused-argument
        ...
        :param turn_context: The :class:`botbuilder.core.TurnContext` for the
    ...
    async def end_component(
        ...
        .. remarks::
            ...
            If the task is successful, the result indicates that the dialog
            ended after the turn was processed by the dialog.

            In general, the parent context is the dialog or bot turn handler
            that started the dialog.
            If the parent is a dialog, the stack calls the parent's
            :meth:`Dialog.resume_dialog()` method to return a result to the
            parent dialog. If the parent dialog does not implement
            :meth:`Dialog.resume_dialog()`, then the parent will end, too, and
            the result is passed to the next parent context, if one exists.

            The returned :class:`DialogTurnResult`contains the return value in
            its :class:`DialogTurnResult.result` property.

        :param outer_dc: The parent class:`DialogContext` for the current turn
        of conversation.
        :type outer_dc: class:`DialogContext`
        :param result: Optional, value to return from the dialog component to
        the parent context.
        :type result: object
