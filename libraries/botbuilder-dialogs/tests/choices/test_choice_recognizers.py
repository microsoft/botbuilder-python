...
def assert_result(result, start, end, text):
    assert result.start == start, f"Invalid ModelResult.start of '{result.start}' for '{text}' result."
    assert result.end == end, f"Invalid ModelResult.end of '{result.end}' for '{text}' result."
    assert result.text == text, f"Invalid ModelResult.text of '{result.text}' for '{text}' result."


def assert_value(result, value, index, score):
    assert result.type_name == 'value', f"Invalid ModelResult.type_name of '{result.type_name}' for '{value}' value."
    assert result.resolution, f"Missing ModelResult.resolution for '{value}' value."
    resolution = result.resolution
    assert resolution.value == value, f"Invalid resolution.value of '{resolution.value}' for '{value}' value."
...
def assert_choice(result, value, index, score, synonym=None):
    assert result.type_name == 'choice', f"Invalid ModelResult.type_name of '{result.type_name}' for '{value}' choice."
    assert result.resolution, f"Missing ModelResult.resolution for '{value}' choice."
    resolution = result.resolution
    assert resolution.value == value, f"Invalid resolution.value of '{resolution.value}' for '{value}' choice."
    ...
    if synonym:
        assert (resolution.synonym == synonym,
...
class ChoiceRecognizersTest(aiounittest.AsyncTestCase):
    ...
    def test_should_find_multiple_values_that_overlap(self):
        found = Find.find_values('the bread pudding and bread please.', _overlapping_values)
        assert len(found) == 2, f"Invalid token count of '{len(found)}' returned."
    ...
    def test_should_correctly_disambiguate_between_similar_values(self):
        found = Find.find_values('option B', _similar_values, FindValuesOptions(allow_partial_matches=True))
        assert len(found) == 1, f"Invalid token count of '{len(found)}' returned."
    ...
    def test_should_find_multiple_choices_that_overlap(self):
        found = Find.find_choices('the bread pudding and bread please.', _overlapping_choices)
        assert len(found) == 2, f"Invalid token count of '{len(found)}' returned."
        assert_result(found[0], 4, 16, 'bread pudding')
