...
from botbuilder.schema import Activity, ActivityTypes, ChannelAccount, ConversationAccount, InputHints, TokenResponse
...
class OAuthPromptTests(aiounittest.AsyncTestCase):
    ...
    async def test_should_call_oauth_prompt(self):
        ...
        async def inspector(activity: Activity, description: str = None):
            ...
            adapter.add_user_token(connection_name, activity.channel_id, activity.recipient.id, token)

            event_activity = create_reply(activity)
            event_activity.type = ActivityTypes.event
            event_activity.from_property, event_activity.recipient = event_activity.recipient, event_activity.from_property
