...
class MainDialog(ComponentDialog):
    def __init__(self, config: DefaultConfig):
        ...
        self.add_dialog(
            WaterfallDialog(
                WaterfallDialog.__name__, [self.sign_in_step, self.show_token_response]
            )
        )
        self.add_dialog(
            OAuthPrompt(
    ...
    async def show_token_response(
        ...
        if not result:
            await context.context.send_activity(
                MessageFactory.text("Skill: No token response from OAuthPrompt")
            )
        else:
            await context.context.send_activity(
                MessageFactory.text(f"Skill: Your token is {result.token}")
            )
