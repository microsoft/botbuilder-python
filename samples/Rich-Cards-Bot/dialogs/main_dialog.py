...
from botbuilder.schema import (ActionTypes, Attachment, AnimationCard, AudioCard, HeroCard, VideoCard,
                              ReceiptCard, SigninCard, ThumbnailCard, MediaUrl, CardAction, 
                              CardImage, ThumbnailUrl, Fact, ReceiptItem)
...
class MainDialog(ComponentDialog):
    ...
    async def choice_card_step(self, step_context: WaterfallStepContext):
        menu_text = ('Which card would you like to see?\n'
                    '(1) Adaptive Card\n'
                    '(2) Animation Card\n'
                    '(3) Audio Card\n'
                    '(4) Hero Card\n'
        ...
        return await step_context.prompt('TextPrompt', PromptOptions(prompt = MessageFactory.text(menu_text)))
    ...
    async def show_card_step(self, step_context: WaterfallStepContext):
        ...
        choice_dict = {
            ...
            '3': [self.create_audio_card], 'audio card': [self.create_audio_card],
            '4': [self.create_hero_card], 'hero card': [self.create_hero_card],
            '5': [self.create_receipt_card], 'receipt card': [self.create_receipt_card],
            '6': [self.create_signin_card], 'signin card': [self.create_signin_card],
            '7': [self.create_thumbnail_card], 'thumbnail card': [self.create_thumbnail_card],
            '8': [self.create_video_card], 'video card': [self.create_video_card],
            '9': [self.create_adaptive_card, self.create_animation_card, self.create_audio_card, self.create_hero_card,
                self.create_receipt_card, self.create_signin_card, self.create_thumbnail_card, self.create_video_card],
            'all cards': [self.create_adaptive_card, self.create_animation_card, self.create_audio_card, self.create_hero_card,
                        self.create_receipt_card, self.create_signin_card, self.create_thumbnail_card, self.create_video_card]
        }
        ...
        if not choice:
            ...
            for func in choice:
                card = func()
                response = create_activity_reply(step_context.context.activity, '', '', [card])
