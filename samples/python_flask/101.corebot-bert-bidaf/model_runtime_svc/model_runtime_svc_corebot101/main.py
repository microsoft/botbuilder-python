...
define('bidaf_model_dir', default=os.path.abspath(os.path.join(HOME_DIR, 'models/bidaf')),
       help='bidaf model directory')
define('bert_model_dir', default=os.path.abspath(os.path.join(HOME_DIR, 'models/bert')),
...
def setup_logging():
    ...
    if not os.path.exists('logs'):
        os.mkdir('logs')
    handler = RotatingFileHandler(os.path.join('./logs', 'model-runtime.log'),
...
def run():
    ...
    if language_helper.initialize_models(options.bert_model_dir, options.bidaf_model_dir) is False:
