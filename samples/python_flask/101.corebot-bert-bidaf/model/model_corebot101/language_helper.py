...
class LanguageHelper:
    ...
    def initialize_models(self, bert_model_dir: str = bert_model_dir_default, bidaf_model_dir: str = bidaf_model_dir_default) -> bool:
        """ Initialize models.
        Perform initialization of the models.
        """
        if not BidafModelRuntime.init_bidaf(bidaf_model_dir, download_ntlk_punkt=True):
            print(f'bidaf model creation failed at model directory {bidaf_model_dir}..', file=sys.stderr)
            return False

        if not BertModelRuntime.init_bert(bert_model_dir):
            print('bert model creation failed at model directory {bert_model_dir}..', file=sys.stderr)
            return False
        ...
        self._bert_intents = BertModelRuntime(model_dir=bert_model_dir, label_list=["Book flight", "Cancel"])
    ...
    async def excecute_query_inproc(self, utterance: str) -> BookingDetails:
        ...
        if intent == 'Book flight':
            ...
            if 'from' in entities:
                print(f'   Recognized "from" entitiy: {entities["from"]}.', file=sys.stderr)
                booking_details.origin = entities['from']
            if 'date' in entities:
                # This value will be a TIMEX. And we are only interested in a Date so grab the first result and drop the Time part.
                # TIMEX is a format that represents DateTime expressions that include some ambiguity. e.g. missing a Year.
                print(f'   Recognized "date" entitiy: {entities["date"]}.', file=sys.stderr)
    ...
    @staticmethod
    async def excecute_query_service(configuration: dict, utterance: str) -> BookingDetails:
        ...
        if lu_response.status_code == 200:
            ...
            entities = await LanguageHelper.validate_entities(response_json['entities'] if 'entities' in response_json else None)
            if intent:
                if 'to' in entities:
                    print(f'   Recognized "to" entity: {entities["to"]}.', file=sys.stderr)
                    booking_details.destination = entities['to']
                if 'from' in entities:
                    print(f'   Recognized "from" entity: {entities["from"]}.', file=sys.stderr)
                    booking_details.origin = entities['from']
                if 'date' in entities:
                    # This value will be a TIMEX. And we are only interested in a Date so grab the first result and drop the Time part.
                    # TIMEX is a format that represents DateTime expressions that include some ambiguity. e.g. missing a Year.
                    print(f'   Recognized "date" entity: {entities["date"]}.', file=sys.stderr)
                    travel_date = entities['date']
                    if await LanguageHelper.validate_timex(travel_date):
                        booking_details.travel_date = travel_date
        return booking_details
    ...
    @staticmethod
    async def call_model_runtime(configuration: Dict[str, object], text: str) -> requests.Request:
