...
class MainDialog(ComponentDialog):
    ...
    def __init__(self, configuration: dict, dialog_id: str = None,
        ...
        wf_dialog = WaterfallDialog('WFDialog', [
            self.intro_step,
            self.act_step,
            self.final_step
        ])
    ...
    async def intro_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        """Initial prompt."""
        if (not self._configuration.get("LUIS_APP_ID", "") or
                not self._configuration.get("LUIS_API_KEY", "") or
                not self._configuration.get("LUIS_API_HOST_NAME", "")):
            await step_context.context.send_activity(
                MessageFactory.text("NOTE: LUIS is not configured. To enable all"\
            ...
            return await step_context.prompt(TextPrompt.__name__,
                PromptOptions(prompt=MessageFactory.text("What can I help you with today?"))) # pylint: disable=bad-continuation
