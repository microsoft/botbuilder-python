...
import random
from botbuilder.core import CardFactory, MessageFactory, TurnContext, UserState, ConversationState, PrivateConversationState
...
class TeamsMessagingExtensionsActionBot(TeamsActivityHandler):
    ...
    async def create_card_command(self, turn_context: TurnContext, action: MessagingExtensionAction
    ) -> MessagingExtensionActionResponse:        
        ...
        attachment = MessagingExtensionAttachment(content=card, content_type=CardFactory.content_types.hero_card, preview=cardAttachment)
        attachments = [attachment]

        extension_result = MessagingExtensionResult(attachment_layout="list", type="result", attachments=attachments)
        return MessagingExtensionActionResponse(compose_extension=extension_result)

    async def share_message_command(self, turn_context: TurnContext, action: MessagingExtensionAction
    ) -> MessagingExtensionActionResponse:
        ...
        if not action.message_payload.attachments is None:
            ...
            card.subtitle = f'({len(action.message_payload.attachments)} Attachments not included)'

        # This Messaging Extension example allows the user to check a box to include an image with the
        # shared message.  This demonstrates sending custom parameters along with the message payload.
        include_image = action.data["includeImage"]
        if include_image == "true":
            image = CardImage(url="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQtB3AwMUeNoq4gUBGe6Ocj8kyh3bXa9ZbV7u1fVKQoyKFHdkqU")
            card.images = [image]
