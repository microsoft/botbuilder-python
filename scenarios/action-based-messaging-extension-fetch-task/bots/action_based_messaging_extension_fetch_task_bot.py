...
class ActionBasedMessagingExtensionFetchTaskBot(TeamsActivityHandler):
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        if value is not None:
            ...
            reply = MessageFactory.text(f"{turn_context.activity.from_property.name} answered '{answer}' and chose '{choices}'.")
            await turn_context.send_activity(reply)
        else:
            # This is a regular text message.
            reply = MessageFactory.text("Hello from ActionBasedMessagingExtensionFetchTaskBot.")
            await turn_context.send_activity(reply)
    ...
    async def on_teams_messaging_extension_submit_action(  # pylint: disable=unused-argument
        ...
        preview_card = self._create_adaptive_card_preview(user_text=question, is_multi_select=multi_select, option1=option1, option2=option2, option3=option3)

        extension_result = MessagingExtensionResult(
            type="botMessagePreview", activity_preview=MessageFactory.attachment(preview_card)
        )
